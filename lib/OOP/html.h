...
<div id="Wifi" class="show"> <!--Login Wifi--> 
  ...
  <script>
    ...
    function onMessage(event){
      ...
      if(String(pre_data).split("/")[0].indexOf("Wifi") < 0 )
        document.getElementById("NameTree").defaultValue = String(pre_data).split("/")[0];
      if(String(pre_data).split("/")[1].indexOf("1") >= 0)
        document.getElementById("DHT11_Status").innerHTML = "Error";
      if(String(pre_data).split("/")[1].indexOf("0") >= 0)
        document.getElementById("DHT11_Status").innerHTML = "Ok";
      if(String(pre_data).split("/")[2].indexOf("1") >= 0)
        document.getElementById("Ligh_Status").innerHTML = "Error";
      if(String(pre_data).split("/")[2].indexOf("0") >= 0)
        document.getElementById("Ligh_Status").innerHTML = "Ok";
      if(String(pre_data).split("/")[3].indexOf("1") >= 0)
        document.getElementById("Mois_Status").innerHTML = "Error";
      if(String(pre_data).split("/")[3].indexOf("0") >= 0)
        document.getElementById("Mois_Status").innerHTML = "Ok";
      Gauge2("humi","c_humi",Number(String(pre_data).split("/")[4]));
      Gauge2("temp","c_temp",Number(String(pre_data).split("/")[5]));
      Gauge1("ligh","c_ligh",Number(String(pre_data).split("/")[6]));
      Gauge1("mois","c_mois",Number(String(pre_data).split("/")[7]));
      if(String(pre_data).split("/")[8].indexOf("1") >= 0)
        document.getElementById("Led").innerHTML = "Led: ON";
      if(String(pre_data).split("/")[8].indexOf("0") >= 0)
        document.getElementById("Led").innerHTML = "Led: OFF";
      if(String(pre_data).split("/")[9].indexOf("1") >= 0)
        document.getElementById("Pump").innerHTML = "Pump: ON";
      if(String(pre_data).split("/")[9].indexOf("0") >= 0)
        document.getElementById("Pump").innerHTML = "Pump: OFF";

      if(String(pre_data).split("/")[10].indexOf("Wifi") >= 0 || String(pre_data).split("/")[0].indexOf("Wifi") >= 0)
      {
        var wifi;
        if(String(pre_data).split("/")[10].indexOf("Wifi") >= 0)
          wifi = String(pre_data).split("/")[10]
        else
          wifi = String(pre_data).split("/")[0]
        if(wifi.indexOf("OFF") >= 0)
        {
        ...
        if(wifi.indexOf("ON") >= 0 || wifi.indexOf("CONNECT") >= 0)
        {
